...
> **核心原则**：先在独立 skill 目录开发，再同步到脚手架，最后更新 README 并等待用户确认。
...
开发流程：
    │
    ├── 1. 独立 skill 目录开发
    │   ├── Claude: skills.claude/<skill>/.claude/skills/<skill>/
    │   ├── Codex: skills.codex/<skill>/.codex/skills/<skill>/
    │   └── Gemini: skills.gemini/<skill>/.gemini/skills/<skill>/
    │   └── 包括：SKILL.md + scripts/ + templates/ + references/
    │
    ├── 2. 同步到脚手架
    │   ├── 复制到 package/学术脚手架/.claude/skills/<skill>/
    │   ├── 复制到 package/学术脚手架/.codex/skills/<skill>/
    │   └── 复制到 package/学术脚手架/.gemini/skills/<skill>/
    │   └── 注意：scripts/ 和 templates/ 属于具体 skill，不在脚手架根目录
    │
    ├── 3. 更新 README
    │   ├── 更新脚手架 README.md
    │   └── 更新 skill 内的说明文档
...
1. **开发顺序**：独立 skill 目录 → 脚手架 → README → 用户确认 → commit
2. **脚本归属**：scripts/ 和 templates/ 属于具体 skill，不在脚手架根目录
3. **同步原则**：脚手架中的 skill 如果版本落后，直接复制替换
4. **Commit 控制**：代理只负责 commit，推送由用户执行
