...
### 硬件感知协议 (Hardware Awareness Protocol)

> **Advisory 模式**：自动探测硬件、生成配置文件、警告资源超限，但不阻塞执行。

**HARDWARE_CONFIG.md 内容**:
```markdown
...
| GPU | {nvidia-smi or "None"} |
| Max Workers | {min(cores-1, 16)} |
| Max Memory (total, 80%) | {RAM * 0.8} GB |
| future.globals.maxSize | {max_memory / max_workers * 1024^3} bytes |
```
...
hw <- list(
  ...
  available_ram_gb = tryCatch({
    if (.Platform$OS.type == "windows") memory.limit() / 1024
    else as.numeric(system("free -g | awk '/Mem/{print $2}'", intern=TRUE))
  }, error = function(e) 64),
  gpu = tryCatch(system("nvidia-smi --query-gpu=name --format=csv,noheader", intern=TRUE),
                 error = function(e) "None")
)
hw$max_workers <- min(hw$cpu_cores - 1, 16)
hw$future_globals_maxsize <- (hw$available_ram_gb * 0.8 / hw$max_workers) * 1024^3
```
...
max_workers <- as.integer(gsub(".*\\| (\\d+).*", "\\1", grep("Max Workers", hw_lines, value=TRUE)))
...
| 可调用 | deep-research | 实验结果需要文献验证(W3) |
