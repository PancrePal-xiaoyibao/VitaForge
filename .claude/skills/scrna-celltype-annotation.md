...
2. 若列名不符合常规命名：
   - 在对话中明确记录真实列名
   - 后续引用时统一采用这些列名
3. 按 cluster 分组，构建每个 cluster 的 marker 概要，**必须保证每个 cluster 至少有 30 个 log2FC 较高的上调基因被纳入后续分析**：
   - 第一步：先按显著性过滤上调基因（例如 `p_val_adj < 0.05` 且 `avg_log2FC > 0`），并按效应量从高到低排序。
   - 若显著上调基因数量 **≥ 30**：可以在这些基因中选取 top N（如 30–100 个，默认 N≈50，根据表格大小与上下文调整）。
   - 若显著上调基因数量 **< 30**：在保留全部显著基因的基础上，继续按 `avg_log2FC` 从高到低补充**未达显著性但 log2FC 较高**的基因，直到该 cluster 至少包含 30 个上调基因。
   - 在后续解释和报告中，要对“仅 log2FC 高但显著性不足的 marker”进行标注，可视为“探索性 marker（非严格显著）”，避免把它们当作高置信度差异基因。
...
1. 一个结构化的 Markdown 报告，包含上述三个主小节。
   - 如果当前环境支持文件写入，你应**优先将这份报告直接写入当前项目根目录**（例如 `./scrna_celltype_annotation_report.md` 或用户指定的文件名），而不是在聊天窗口中整段贴出全文。
   - 在对话中，只需给出：
     - 报告文件路径/文件名
     - 报告结构与主要结论的简短摘要。
   - 之后所有关于注释的微调/修订，原则上都应当**围绕这一个报告文件进行增量修改**，将其视为“注释结果的唯一权威版本”，而不是不断生成新的长篇回复。
2. 在普通对话中简要总结：
   - 一共注释了多少个 cluster
   - major 大类数量及名称
