...
干实验 pipeline 调试与审查：
  code-debugger                     →  R/Python 脚本 debug
  → code-review                     →  分析流程审查
  → codebase-context                →  复杂生信代码导航
```
...
本 skill 为 VitaForge Package **主调度**，拥有全局路由表。所有子 skill 均可被路由。子 skill 之间亦可直接互调，不受树形层级约束（详见 loop-engineer 的网络调度协议）。

## Resources

| 文件 | 用途 | 何时加载 |
|------|------|----------|
| 各子 skill SKILL.md | 确认子 skill 能力边界 | 跨场景编排时 |
| 项目 README.md | 全量 skill 速查 | 用户询问全局能力时 |
