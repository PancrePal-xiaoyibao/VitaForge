# GEMINI.md - VitaForge (Gemini CLI 入口)

> 面向医学-生命科学 AI4S 的干实验引擎。本仓库为 Gemini CLI 提供 skills（见 `.gemini/skills/`），Gemini 会根据任务描述自动匹配并激活对应 skill（渐进式披露：元数据常驻，全文按需加载）。

## 🚀 主调度入口

- **`vitaforge-orchestrator`** — VitaForge 全场景分诊调度器（**推荐首选入口**）。描述你的医学/生命科学/科研需求，自动识别场景并路由到最合适的子 skill。不确定用哪个 skill 时，先描述需求让 Gemini 匹配这个主调度。

## 📋 技能清单（`.gemini/skills/` 下，Gemini 自动匹配触发）
...
Gemini 官方发现路径：`.gemini/skills/<name>/` 或 `.agents/skills/<name>/`（别名，优先级更高）。
也可用 `gemini skills link /path/to/.gemini` 批量符号链接，或 `gemini skills install` 从 Git 安装。
